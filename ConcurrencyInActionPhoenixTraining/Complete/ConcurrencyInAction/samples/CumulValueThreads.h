...
namespace Ex30
{
	...
	void CleanupDeadThreads(std::vector<std::future<int>*>& tasksPool)
	{
		sleep(100);
		for (std::future<int>*& future : tasksPool)
		{
			if (future && future_is_ready(*future))
			{
				delete future;
				future = nullptr;
			}
	...
	void CleanupDeadThread(std::vector<std::future<int>*>& tasksPool, int index)
	{
		std::future<int>*& future = tasksPool[index];
	...
	void HandleFinishedTasks(std::vector<std::future<int>*>& tasksPool, int& cumul, int& currentFinishedTasks, int currentTask)
	{
		...
		while (!g_waitForDeadFutureTaks.empty())
		{
			...
			CleanupDeadThread(tasksPool, indexIntasksPool);
		}
	}
